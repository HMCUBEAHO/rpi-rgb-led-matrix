...
void runExample(int userInput) {
    ...
    const char *baseCommand = "sudo ./demo --led-rows=64 --led-cols=64 --led-chain=6 --led-gpio-mapping=adafruit-hat --led-no-hardware-pulse --led-slowdown-gpio=4 --led-brightness=40 --led-pixel-mapper=""U-mapper;Rotate:90"" -D ";
